...
template<class T>
class FCGenerator : public Generator<FCGenerator<T>> {
public:
  typedef Generator<FCGenerator<T>> Base;
  using Base::get_target;
  using Base::natural_vector_size;
  using Base::target;
  template<typename T2>
  ...
  GeneratorParam<int> in_size_ = {"in_size", 1024};
  GeneratorParam<int> out_size_ = {"out_size", 1024};
  ...
  void generate() {
    ...
    const int vec_factor = vectorize_ ? natural_vector_size(type_of<T>()) : 1;
    ...
    const Expr output_size = out_size_;//A_.height(); // 1 dimension (output dim)

    const Expr tail_size = out_size_ % (unroll_factor*vec_factor);
    const Expr unrolled_size = out_size_ - tail_size;

    // input dim
    RDom k(0, unrolled_size, "k");
...
HALIDE_REGISTER_GENERATOR(FCGenerator<float>, sfc)
HALIDE_REGISTER_GENERATOR(FCGenerator<double>, dfc)
